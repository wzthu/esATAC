...
#load("Report.Rdata")
library(esATAC)
allsteps.info <- readRDS("allsteps.info.rds")
allsteps <- allsteps.info[["prevSteps"]]
wholesummary <- allsteps.info[["wholesummary"]]
filtstat <- allsteps.info[["filtstat"]]
```
...
knitr::kable(x = report(allsteps$LibComplexQC)[["report"]])
```
...
```{r cols.print=2, eval=TRUE, echo=FALSE,warning=FALSE}
ifelse(is.null(allsteps$PeakQCblacklist),"NA",
knitr::kable(x = report(allsteps$PeakQCblacklist)[["report"]]))
```

## DHS ratio

```{r cols.print=2, eval=TRUE, echo=FALSE,warning=FALSE}
ifelse(is.null(allsteps$PeakQCDHS),"NA",
knitr::kable(x = report(allsteps$PeakQCDHS)[["report"]]))
```

## Fraction of reads in peaks (FRiP)

```{r cols.print=2, eval=TRUE, echo=FALSE,warning=FALSE}
knitr::kable(x = report(allsteps$FRiPQC)[["report"]])
```
