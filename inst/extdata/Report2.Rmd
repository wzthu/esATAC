...
The total number in case is `r PeakNumInCC[4]`, specific in case is `r PeakNumInCC[1]`.

The total number in control is `r PeakNumInCC[5]`, specific in control is `r PeakNumInCC[2]`.

The overlap peak are merged to `r PeakNumInCC[3]` peaks.
...
This function takes case and control specific peaks as foreground respectively, overlap peaks as background.

## motif enrich in case specific peak regions 
...
if(nrow(motif_enrich.case) < 15){
    knitr::kable(motif_enrich.case)
}else{
    knitr::kable(motif_enrich.case[1:15, ])
}
```

## motif enrich in control specific peak regions
...
if(nrow(motif_enrich.ctrl) < 15){
    knitr::kable(motif_enrich.ctrl)
}else{
    knitr::kable(motif_enrich.ctrl[1:15, ])
}
```
...
pdf.dir <- getReportVal(comp_result$footprint.case, "pdf.dir")
pdf.dir <- getReportVal(comp_result$footprint.ctrl, "pdf.dir")
```
