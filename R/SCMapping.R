setClass(Class = "SCBowtie2Mapping",
         contains = "ATACProc"
)
...
#' you can use \code{scBowtie2Mapping} instead.
...
setGeneric("atacSCBowtie2Mapping",function(atacProc,samOutput=NULL,
                                         ...
                                         standardGeneric("atacSCBowtie2Mapping"))



#' @rdname SCBowtie2Mapping
#' @aliases atacSCBowtie2Mapping
#' @export
setMethod(
    f = "atacSCBowtie2Mapping",
    signature = "ATACProc",
    definition = function(atacProc,samOutput=NULL,
                          ...
                          interleave = FALSE, threads = getThreads(), 
                          paramList="--no-discordant --no-unal --no-mixed -X 2000", ...){
        allpara <- c(list(Class = "SCBowtie2Mapping", prevSteps = list(atacProc)),as.list(environment()),list(...))
        step <- do.call(new,allpara)
        invisible(step)
    }
)
#' @rdname SCBowtie2Mapping
#' @aliases scBowtie2Mapping
#' @export
scBowtie2Mapping <- function(fastqInput1, fastqInput2=NULL,
                           samOutput=NULL,reportOutput =NULL, 
                           bt2Idx=NULL, interleave = FALSE, 
                           threads = getThreads(), 
                           paramList="--no-discordant --no-unal --no-mixed -X 2000", ...){
    allpara <- c(list(Class = "SCBowtie2Mapping", prevSteps = list()),as.list(environment()),list(...))
    step <- do.call(new,allpara)
    invisible(step)
}
