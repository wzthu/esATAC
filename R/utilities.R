...
############################Scan Motif and Save result as RDS###################
#' @name scanGenomeMotif
#' @title Scan motif position.
#' @description
#' Scan motif position from the given genome.
#' @param motifPWM \code{list} scalar.
...
#'
#' @export
scanGenomeMotif <- function(motifPWM = NULL, refgenome = NULL,
                            min.score = NULL, n.core = NULL, output = NULL){
    names(motifPWM) <- gsub(pattern = "[^a-zA-Z0-9]", replacement = "", x = names(motifPWM), perl = TRUE)
    ...
    sitesetList <- parLapply(cl = cl,
                             ...
                             with.score = TRUE)
    stopCluster(cl)
    saveRDS(object = sitesetList, file = output)
}
