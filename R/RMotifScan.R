RMotifScan <- R6::R6Class(
  ...
  private = list(
    processing = function(){
      ...
      sitesetList <- TFBSTools::searchSeq(x = pwm, subject = ref)
...
MotifScan <- function(atacProc = NULL, Seq_file = NULL,
                      Motif = NULL, output_file = NULL, Position = FALSE, MP_file = NULL){
  tmp <- RMotifScan$new(atacProc, Seq_file, Motif, output_file, Position)
  tmp$process()
  return(tmp)
}
