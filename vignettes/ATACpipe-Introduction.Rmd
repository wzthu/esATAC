...
For example,if you want to query "SamToBed" in the flowchart, 
you can add "atac" prefix to query mannual like this:
...
* hg19.fa
* hg19.DHS.bed
* hg19.knownGene.sqlite
* hg19.blacklist.bed

NOTE: genome.1.bt2, genome.2.bt2, genome.3.bt2, genome.4.bt2, genome.rev.1.bt2, genome.rev.2.bt2 are bowtie2 index files. It takes long time to build bowtie2 index from genome.fa file so you can put them into reference installation path mannually. genome.fa is the reference genome sequence. It will be generated from BSgenome pakcage(e.g. BSgenome.Hsapiens.UCSC.hg19) in several minutes rather than download from internet. genome.DHS.bed, genome.knownGene.sqlite, genome.blacklist.bed are small files, which will be automatically generated or download in short time. 
...
conclusion2 <- atacPipe2(case=list(fastqInput1 = case[1],fastqInput2 = case[2]),
...
To run the whole pipeline for a typical ATACseq data set of human sample may take ~2 days on a personal computer with single thread. If your thread or R has been stopped during the process. You can simplely resume the analysis by retypin your command line.
