...
**Note: By default, esATAC will build bowtie2 index if there is no bowtie2 index, which may take several hours with single thread. If you want to download or use your own bowtie2 index instead, please see [section 2.2](#bt2)**
...
They will not be downloaded and installed during executing `biocLite("esATAC")` because these package file are very big. They will be downloaded and installed according to the `genome` parameter when calling the preset pipeline for the first time.
...
## How to use Preset motif data for esATAC

Motif scan is a time-consuming step in ATAC-seq data analysis. We provide functions and data for accelerate this step. We provide Vertebrata motifs from [JASPAR database](http://www.bioconductor.org/packages/release/data/experiment/html/JASPAR2016.html) and use "scanGenomeMotif" function to scan motif information in the genome. Users can use these data to plot footprint. [Click here for download.](https://github.com/A1dnoah/esATAC_preset_motif_data)

The following is the tutorial about how to use esATAC to build preset motif information data and use this data to run ATAC-seq data analysis pipeline.

### How to generate motif data of hg19
...
names(pwm) <- gsub(pattern = "[^a-zA-Z0-9]", replacement = "", x = names(pwm), perl = TRUE)  # remove special characters
scanGenomeMotif(motifPWM = pwm, refgenome = BSgenome.Hsapiens.UCSC.hg19,
                min.score = "90%", n.core = 12, output = "JASPAR_verteb_hg19.rds")
```

Of course, if you want to use another PWM and genome, just change the parameters in "scanGenomeMotif". For more information, please see [*esATAC Reference Manual*](http://www.bioconductor.org/packages/release/bioc/manuals/esATAC/man/esATAC.pdf). 

### How to use this data when analysis ATAC-seq data

```{r eval = FALSE, echo = TRUE, warning = FALSE, results = 'hide', message = FALSE}
options(java.parameters = "-Xmx8192m")  # set Java parameter
library(esATAC)

# ATAC-seq data, case&control analysis
case = list(fastqInput1="ntest1_1.fastq.bz2",fastqInput2="ntest1_2.fastq.bz2",
            adapter1 = NULL, adapter2 = NULL)
control =list(fastqInput1="ntest2_1.fastq.bz2",fastqInput2="ntest2_2.fastq.bz2",
              adapter1 = NULL, adapter2 = NULL)
              
results <- atacPipe2(case = case,
                     control = control,
                     refdir = "path_to_bowtie2_index",
                     genome = "hg19",
                     tmpdir = "path_to_save_temp_data",
                     threads = 4,
                     chr = c(1:22, "X", "Y"), use.SavedPWM = "JASPAR_verteb_hg19.rds")
```
...
## Gallery for Analysis Result of Real ATAC-seq Dataset

We collect some representative ATAC-seq datasets and process them with matched pipelines.
